...
attachments = {}
refered_web = {"user": [], "domain": [], "d_count": []}
popular_words = {}
chat_words = {}
emojis = {"user": [], "emj_char": [], "char_count": []}
heat_map = {
    "user": [],
    "day": [], 
    "hour": [], 
...
EMOJI
"""
def extract_emojis(member, string):
    emj = []
    for c in string:
    ...
    if len(emj) > 0:
        increment_emoji_count(member, emj)
...
def rank_word(member, word):
    
    if not word in common_words:
        popular_words[word] = popular_words.get(word, 0) + 1
        global chat_words
        chat_words[member] = chat_words.get(member, "") +  " {0}".format(word)
...
def increment_link_count(member, urls):
    for url in urls:
        increment_domain_count(member, get_domain(url))
                    
    return len(urls)
    
def increment_domain_count(member, domain):
    refered_web["user"].append(member)
    refered_web["domain"].append(domain)
    refered_web["d_count"].append(1)

def increment_emoji_count(member, char_list):
    groups = Counter(char_list)
    for c in groups.items():
        emojis["user"].append(member)
        emojis["emj_char"].append(c[0])
        emojis["char_count"].append(c[1])
        
def increment_heatmap(member, dt):
    heat_map["user"].append(member)
...
def parse_line(line, verbose):
    ...
    if is_starting:
        ...
        if chat:
            ...
            member = chat.group(1)
            increment_heatmap(member, dt)

            increment_chat_count(member)
            ...
            if has_attachment:
                ...
                if is_deleted(message_body):
                    ...
                    if urls:
                        increment_link_count(member, urls)

                    #Emoji
                    emjs = extract_emojis(member, message_body)
                    
                    #Word Count & Wordcloud
                    get_words(member, message_body)
        ...
        #Emoji    
        emjs = extract_emojis(member, line)
        
        #Word Count & Wordcloud
        get_words(member, line)
...
emojis_df = (pd.DataFrame(emojis)
                     .groupby(["user", "emj_char"])
                     .sum()
                     .sort_values(by = ['char_count'], ascending=False))
top_repeated_emoji = (pd.DataFrame(emojis)
                     .sort_values(by = ['char_count'], ascending=False))
heatmap_df = pd.DataFrame(heat_map)
grouped_heatmap = heatmap_df.groupby(["user", "day", "hour"]).sum().sort_values(by=["chat_count"], ascending=False)
...
"""
if chat_words:
    for member, chats in chat_words.items():
        wordcloud = WordCloud(
            width = 1000,
            height = 500,
        ...
        plt.suptitle(member, fontsize=20)
